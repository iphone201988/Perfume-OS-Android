package com.tech.perfumos.utils.seekbar;
...
public class TickSeekBar extends View {
    ...
    private Drawable mBackgroundTrackDrawable;
    private Bitmap mBackgroundTrackBitmap;
    private int mProgressTrackColor;
    private Drawable mProgressTrackDrawable;
    private Bitmap mProgressTrackBitmap;
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        ...
        //track
        mBackgroundTrackDrawable=ta.getDrawable(R.styleable.TickSeekBar_tsb_track_background_drawable);
        mProgressTrackDrawable=ta.getDrawable(R.styleable.TickSeekBar_tsb_track_progress_drawable);
...
//        initTextsTypeface(ta.getInt(R.styleable.TickSeekBar_tsb_tick_texts_typeface, -1), builder.tickTextsTypeFace);
        Typeface fontFamilyTypeface = null;
        if (ta.hasValue(R.styleable.TickSeekBar_tsb_tick_texts_fontFamily)) {
            int fontResId = ta.getResourceId(R.styleable.TickSeekBar_tsb_tick_texts_fontFamily, 0);
            if (fontResId != 0) {
                fontFamilyTypeface = ResourcesCompat.getFont(context, fontResId);
            }
        }
        if (fontFamilyTypeface != null) {
            mTextsTypeface = fontFamilyTypeface;
        } else {
...
//        initTickXAndTextXLocation();
    ...
    private void drawTrack(Canvas canvas) {
        if (mBackgroundTrackDrawable!=null && mProgressTrackDrawable!=null){
            if (mBackgroundTrackBitmap == null || mProgressTrackBitmap == null) {
                initTrackBitmap();
            }
            if (mBackgroundTrackBitmap == null || mProgressTrackBitmap == null) {
            ...
            float bgLeft = mBackgroundTrack.left;
            float bgTop = mBackgroundTrack.top - mBackgroundTrackBitmap.getHeight() / 2f;
            canvas.drawBitmap(mBackgroundTrackBitmap, bgLeft, bgTop, mStockPaint);

            // Draw progress track (up to thumb)
            float thumbCenterX = getThumbCenterX();
            float progressWidth = thumbCenterX - mProgressTrack.left;
            Rect src = new Rect(0, 0, (int) progressWidth, mProgressTrackBitmap.getHeight());
            RectF dst = new RectF(mProgressTrack.left, mProgressTrack.top - mProgressTrackBitmap.getHeight() / 2f,
                    thumbCenterX, mProgressTrack.top + mProgressTrackBitmap.getHeight() / 2f);
            canvas.drawBitmap(mProgressTrackBitmap, src, dst, mStockPaint);
        }
        else if (mCustomTrackSectionColorResult) {//the track has custom the section track color
    ...
    private void drawTickTexts(Canvas canvas) {
        ...
        for (int i = 0; i < mTickTextsArr.length; i++) {
            ...
            if (i == 0) {
                ...
                Log.d("mTickTextsArr", "drawTickTexts:  "+mTickTextsArr[index]);
                canvas.drawText(mTickTextsArr[index], mTextCenterX[i] - mTickTextsWidth[index] / 2.0f, mTickTextY, mTextPaint);

            } else {
                Log.d("mTickTextsArr", "drawTickTexts1:  "+mTickTextsArr[index]);
                canvas.drawText(mTickTextsArr[index], mTextCenterX[i], mTickTextY, mTextPaint);

                /*String[] tickLines = mTickTextsArr[i].split("\\n+");
                float centerX = mTextCenterX[i];
                float y = mTickTextY; // base Y for tick text

                for (int line = 0; line < tickLines.length; line++) {
                    mTextPaint.getTextBounds(tickLines[line], 0, tickLines[line].length(), mRect);
                    float yOffset = y + line * (mRect.height()); // 6px line spacing
                    canvas.drawText(tickLines[line], centerX - mRect.width() / 2.0f, yOffset, mTextPaint);
                }*/
            }
        }
    }
    ...
    private void initTrackBitmap() {
        if (mBackgroundTrackDrawable == null) {
            return;
        }
        if (mBackgroundTrackDrawable instanceof StateListDrawable) {
            try {
                StateListDrawable listDrawable = (StateListDrawable) mBackgroundTrackDrawable;
                Class<? extends StateListDrawable> aClass = listDrawable.getClass();
                int stateCount = (int) aClass.getMethod("getStateCount").invoke(listDrawable);
                if (stateCount == 2) {
                    Method getStateSet = aClass.getMethod("getStateSet", int.class);
                    ...
                    for (int i = 0; i < stateCount; i++) {
                        ...
                        if (stateSet.length > 0) {
                            if (stateSet[0] == android.R.attr.state_pressed) {
                                Drawable stateDrawable = (Drawable) getStateDrawable.invoke(listDrawable, i);
                                mBackgroundTrackBitmap = getDrawBitmap(stateDrawable, true);
                            } else {
                                //please check your selector drawable's format, please see above to correct.
                                throw new IllegalArgumentException("the state of the selector thumb drawable is wrong!");
                            }
                ...
                mBackgroundTrackBitmap = getDrawBitmap(mBackgroundTrackDrawable, true);
                mProgressTrackBitmap = mBackgroundTrackBitmap;
            }
        } else {
            mBackgroundTrackBitmap = getDrawBitmap(mBackgroundTrackDrawable, true);
            mProgressTrackBitmap = mBackgroundTrackBitmap;
    ...
    private void initTextPaint() {
        if (mTextPaint == null) {
           ...
           /* Typeface typeface = ResourcesCompat.getFont(mContext, R.font.inter_regular);
            if (typeface != null) {
                mTextPaint.setTypeface(typeface);
            }*/
            if (mTextsTypeface != null) {
                mTextPaint.setTypeface(mTextsTypeface);
            }
